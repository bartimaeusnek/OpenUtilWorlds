package com.github.bartimaeusnek.bettervoidworlds.common.blocks;
...
import static com.github.bartimaeusnek.bettervoidworlds.common.world.dimension.DimensionTypeManager.PROPERTY_ENUM;
...
public class UniversalPortalBlock extends BlockContainer {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (worldIn.provider.getDimension() != toPort) {
            ...
            if (!teData.getBoolean("ReturnHasBeenSet") && !(worldOut.getTileEntity(toSpawn) instanceof UniversalTeleportTE)) {
                ...
                if (worldOut.provider.getDimensionType() != DimensionTypeManager.VOID.getDimensionType()) {
                    while (!worldOut.isAirBlock(toSpawn)) {
                        toSpawn = toSpawn.up();
                    }
                    while (worldOut.isAirBlock(toSpawn)) {
                        toSpawn = toSpawn.down();
                    }
                    toSpawn = toSpawn.up();
                }
                ...
                tagCompound.setIntArray("RETURNPOS", new int[]{pos.getX(), pos.getY(), pos.getZ()});
    ...
    public void harvestBlock(World world, EntityPlayer player, BlockPos pos, IBlockState state, @Nullable TileEntity te, @Nullable ItemStack stack) {
        NBTTagCompound tag = te.getTileData();
        ItemStack ret = new ItemStack(ItemRegistry.portalItemBlock, 1, state.getValue(DimensionTypeManager.PROPERTY_ENUM).getMeta());
        ret.setTagCompound(new NBTTagCompound());
        ret.getTagCompound().merge(tag);
        spawnAsEntity(world, pos, ret);
    }
